...
class PegasusPPOTrainer:
    """
    The PegasusPPOTrainer uses Proximal Policy Optimization to optimise language models.
    """
    ...
    def __init__(self, model, ref_model, **ppo_params):
        ...
        if self.ppo_params['optimizer'] == 'SGD':
    ...
    def batched_forward_pass(self, model_input, gen_len):
        ...
        for i in range(int(self.ppo_params['batch_size'] / fbs)):
            ...
            logprobs.append(
                logprobs_from_logits(logits[:, :-1, :], m_input["decoder_input_ids"][:, 1:])[:, -gen_len:].detach())
            ref_logprobs.append(
                logprobs_from_logits(ref_logits[:, :-1, :], m_input["decoder_input_ids"][:, 1:])[:, -gen_len:].detach())
